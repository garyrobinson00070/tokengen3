...
async function main() {
  try {
    ...
    console.log(`Deploying AntiBotPresaleContract to ${networkName}`);
    console.log("Presale config:", presaleConfig);

    // Get contract factory
    const AntiBotPresaleFactory = await hre.ethers.getContractFactory("AntiBotPresaleContract");
    ...
    const antiBotInfo = {
      protectionDelay: presaleConfig.antiBotConfig?.protectionDelay || 300, // 5 minutes default
      maxGasPrice: hre.ethers.parseUnits(
        (presaleConfig.antiBotConfig?.maxGasPrice || 50).toString(), 
        'gwei'
      ),
      walletCooldown: presaleConfig.antiBotConfig?.walletCooldown || 60, // 1 minute default
      signatureRequired: presaleConfig.antiBotConfig?.signatureRequired || false
    };
    
    // Is this a fairlaunch?
    const isFairlaunch = presaleConfig.saleType === 'fairlaunch';
    ...
    const constructorArgs = [
      ...
      deployer.address, // Signer address for signature verification
      isFairlaunch
    ];
    
    // Deploy contract
    console.log("Deploying presale contract...");
    const presale = await AntiBotPresaleFactory.deploy(...constructorArgs);
    
    // Wait for deployment
    await presale.waitForDeployment();
    const contractAddress = await presale.getAddress();
    
    console.log("AntiBotPresaleContract deployed to:", contractAddress);
    ...
    const deploymentInfo = {
      contractType: "AntiBotPresaleContract",
      contractAddress,
      ...
      constructorArgs: [
        ...
        vestingInfo,
        {
          protectionDelay: antiBotInfo.protectionDelay,
          maxGasPrice: antiBotInfo.maxGasPrice.toString(),
          walletCooldown: antiBotInfo.walletCooldown,
          signatureRequired: antiBotInfo.signatureRequired
        },
        presaleConfig.walletSetup.saleReceiver,
        presaleConfig.walletSetup.refundWallet,
        deployer.address,
    ...
    const deploymentFile = path.join(deploymentsDir, `${networkName}-antibot-presale-${contractAddress}.json`);
